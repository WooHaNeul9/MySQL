...
# UPDATE 테이블명 SET 컬럼명=넣을값 WHERE 조건
# UPDATE 테이블명 SET 컬럼명=넣을값 WHERE 조건 AND 조건

-- 플레이어 이름이 조규성이면 나이를 27, 포지션을 FW, 프로팀을 FC 밀트윌란, 등번호를 10으로 변경
UPDATE player SET age = 27, position = 'FW', pro_team = 'FC 밀트윌란', uniform_number = 10 WHERE name='조규성';

-- 플레이어 이름이 박용우이면 나이를 32, 포지션을 MF, 프로팀을 알 아인 FC, 등번호를 5로 변경
UPDATE player SET age = 32, position = 'MF', pro_team = '알 아인 FC', uniform_number = 5 WHERE name='박용우';

-- 플레이어 이름이 쿠보 타케후사이면서 국적이 일본이면 나이를 24, 포지션을 MF/FW, 프로팀을 레알 소시에다드, 등번호를 20로 변경
...
UPDATE player SET age = 34, position = 'MF', pro_team = '토트넘 홋스퍼', uniform_number = 7 WHERE name = '손흥민';

-- 대한민국 선수 중 30살 이상의 선수 조회
